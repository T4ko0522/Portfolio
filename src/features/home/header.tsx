        ...
        <div className="grid grid-cols-4 gap-1 py-4">
          {navItems.map((item) => {
            ...
            return (
              <motion.button
                key={item.id}
                onMouseEnter={() => setHoveredButtons((prev) => new Set(prev).add(item.id))}
                onMouseLeave={() =>
                ...
                className="font-huninn text-white bg-transparent transition-colors duration-200 relative overflow-x-visible overflow-y-hidden group px-4 py-2 rounded-md flex items-center justify-center"
              >
                <span
