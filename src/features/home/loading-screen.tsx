...
const Loader: React.FC<LoaderProps> = ({ onLoadingComplete }) => {
  const [lines, setLines] = useState<
    Array<{ type: "prompt" | "command" | "output" | "special"; text: string }>
  >([])
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      await typeText("bun run dev", 90)
      addLine("command", "bun run dev")

      // 起動出力をシミュレート
      await delay(300)
      addLine("output", "> portfolio@1.1.0 start")
      await delay(200)
      addLine("output", "> vp dev")
      await delay(400)
      addLine("special", "   ▲ TanStack Start")
      await delay(400)
