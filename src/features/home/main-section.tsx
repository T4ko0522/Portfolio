...
import { introTexts } from "./content"
import OutlineTitle from "./outline-title"
import MobileTypingAnimation from "./mobile/mobile-typing-animation"

interface MainSectionProps {
  isMobile: boolean
}

export default function MainSection({ isMobile }: MainSectionProps) {
  return (
    <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-center pt-32 pb-16">
      ...
      <div className="w-full text-center mb-20 px-4">
        <motion.div
          ...
          className={`font-pacifico ${isMobile ? "text-4xl" : "text-6xl md:text-7xl lg:text-8xl"} font-normal mb-10 flex items-center justify-center gap-4`}
        >
          <Image
            ...
            height={80}
            className={
              isMobile ? "w-12 h-12" : "w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 inline-block"
            }
          />
      ...
      >
        {isMobile ? (
          <MobileTypingAnimation
            texts={introTexts}
            className="font-pacifico text-xl sm:text-4xl md:text-5xl lg:text-6xl"
          />
        ) : (
          <TypingAnimation
            texts={introTexts}
            className="font-pacifico text-4xl md:text-5xl lg:text-6xl"
          />
        )}
      </motion.div>
