...
export default function TypingAnimation({ texts, className = "" }: TypingAnimationProps) {
  ...
  return (
    <div
      className={`flex items-center justify-center gap-x-1.5 sm:gap-x-2 w-full py-2 sm:py-4 ${className}`}
    >
      <div
        className="flex items-center gap-x-1.5 sm:gap-x-2 leading-normal"
      ...
      >
        ...
        <span className="text-white flex items-center" style={{ minHeight: "1.9em" }}>
          ...
          >
            <span className="inline-block whitespace-nowrap">{texts[currentIndex]}</span>
          </span>
          {/* カーソル（overflow の外に配置） */}
          <span
            className="inline-block w-0.5 sm:w-1 bg-white ml-1 sm:ml-2 animate-blink flex-shrink-0"
            style={{ height: "1em" }}
          />
        </span>
      </div>
